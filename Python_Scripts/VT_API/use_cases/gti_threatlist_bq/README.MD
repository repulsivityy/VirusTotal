...
export BUCKET_NAME="gti-ioc-responses-${GCP_PROJECT_ID}"
export BQ_DATASET_NAME="gti_threatintel_bq"
export BQ_TABLE_NAME="malicious_urls"
export BQ_STAGING_TABLE_NAME="malicious_urls_staging"
export SECRET_NAME="gti-api-key"
export FETCH_FUNCTION_NAME="gti-fetch-iocs"
export LOAD_FUNCTION_NAME="gti-load-data"
export MERGE_FUNCTION_NAME="gti-execute-merge"
export FETCH_SCHEDULER_NAME="gti-hourly-trigger"
export MERGE_SCHEDULER_NAME="gti-hourly-merge"
...
bq mk --table \
  --description="Final de-duplicated table for GTI malicious URLs" \
  $GCP_PROJECT_ID:$BQ_DATASET_NAME.$BQ_TABLE_NAME \
  ./schema.json

# Create the staging table
bq mk --table \
  --description="Staging table for incoming GTI malicious URLs" \
...
---
#### 5.1: The "Fetch" Function (`gti-fetch-iocs`)
...
1.  **Create the directory and files:**
    ...
    * `main.py`: Add the Python code found in the `gti-fetch-iocs/main.py` file of this repository.
    * `requirements.txt`: Add the content from `gti-fetch-iocs/requirements.txt`.

2.  **Deploy the function:**
    ```bash
    # Make sure you are inside the gti-fetch-iocs directory
...
---
#### 5.2: The "Load" Function (`gti-load-data`)
...
1.  **Create the directory and files:**
    ...
    ```
    Inside this new directory, create two files:
    * `main.py`: Add the Python code found in the `gti-load-data/main.py` file of this repository.
    * `requirements.txt`: Add the content from `gti-load-data/requirements.txt`.
...
---
#### 5.3: The "Merge" Function (`gti-execute-merge`)
...
1.  **Create the directory and files:**
    ```bash
    mkdir gti-execute-merge
    ```
    Inside this new directory, create two files:
    * `main.py`: Add the Python code found in the `gti-execute-merge/main.py` file of this repository.
    * `requirements.txt`: Add the content from `gti-execute-merge/requirements.txt`.
...
This job runs every hour at 5 minutes past the hour and calls the `gti-fetch-iocs` function.
...
This job runs every hour at 15 minutes past the hour, giving the fetch-and-load process plenty of time to complete before it calls the `gti-execute-merge` function.
...
**Solution**: The final architecture is more reliable as it replaces the Transfer Service with a dedicated, event-driven Cloud Function (`gti-load-data`) that we have full control over.
...
* **Problem 1: `NULL` Values in Table**: The first successful loads inserted rows with all `NULL` values.
    ...
    * **Solution**: The `gti-fetch-iocs` function was updated to meticulously reshape the JSON, renaming and flattening keys to perfectly match the table schema before saving the file.
