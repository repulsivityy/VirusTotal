...
# !!! IMPORTANT: Set your GTI API Key as an environment variable named GTI_APIKEY !!!
# Example: export GTI_APIKEY="your_actual_api_key_here"
...
FILE_SIZE_LIMIT_BYTES = 32 * 1024 * 1024  # 32 MB in bytes
...
def upload_file_to_vt(api_key, filepath, filename):
    ...
    if file_size > FILE_SIZE_LIMIT_BYTES:
        ...
        # Add x-tool header for getting upload URL
        headers_for_get_url = {
            'x-apikey': api_key,
        ...
        try:
            ...
            # Add x-tool header for the actual upload POST to special URL
            actual_upload_headers = {
                'x-apikey': api_key,
                'accept': 'application/json',
                'x-tool': X_TOOL_HEADER
        ...
        # Add x-tool header for the actual upload POST to /files
        actual_upload_headers = {
            'x-apikey': api_key,
            'accept': 'application/json',
            'x-tool': X_TOOL_HEADER
    ...
    try:
        with open(filepath, 'rb') as f:
            ...
            # actual_upload_headers (defined above) now includes x-tool
...
def get_vt_report(api_key, file_hash):
    ...
    # Add x-tool header for getting report
    headers = {
        'x-apikey': api_key,
        'accept': 'application/json',
        'x-tool': X_TOOL_HEADER
