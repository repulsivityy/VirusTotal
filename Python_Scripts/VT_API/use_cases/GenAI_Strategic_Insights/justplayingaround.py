...
Version: 1.2 - Implemented in-memory buffer for daily summaries.
"""
...
import json
import argparse
import io # New import for in-memory buffer

####################
#  Configuration and Initialization 
####################

# Define a maximum token limit for the combined summaries to prevent exceeding Gemini's context window.
# 32,000 is a safe estimate for Gemini-2.5-pro, but this can be adjusted based on testing.
MAX_BUFFER_TOKENS = 32000 
...
def get_system_instruction(output_country, output_language):
    ...
    <PROMPT>
        ...
        <INPUT_FORMAT>
            ...
            4.  **`VULNERABILITY_DETAILS`:** (Optional) A list of structured CVE details, which should be used to create a table at the end of the report.
        </INPUT_FORMAT>
        ...
        3.  **Vulnerability Summary (If Applicable):** * If a `VULNERABILITY_DETAILS` object is provided, you must create a section at the end of the report titled "**Vulnerability Spotlight**" (translated to `{output_language}`).
...
def get_user_prompt(collections, output_country, cve_details=None):
    ...
    collections_subset = collections[:300]
...
def get_final_rollup_prompt(combined_summaries, output_country, cve_details=None):
    """
    Creates the prompt for the final 'roll-up' summary.
    """
    prompt = f"""
    You are a senior threat intelligence analyst. You have been provided with several daily threat intelligence summaries for {output_country}.

    Your task is to combine these daily summaries into a single, cohesive, high-level weekly report. The report should start with a brief executive summary that highlights the most critical and recurring threats over the past few days.

    After the executive summary, present a section for "Key Threat Landscape Developments" that includes the original detailed daily summaries.

    If CVE details are provided, append a "Vulnerability Spotlight" table at the very end of the report.

    Here are the daily summaries to work with:
    {combined_summaries}
    """
...
async def main():
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            # Use an in-memory text buffer to store daily summaries
            daily_summaries_buffer = io.StringIO()
            ...
            for i in range(args.days):
                ...
                daily_reports = await fetch_reports(session, gti_api_key, start_date=day_start, end_date=day_end)
                
                if not daily_reports:
                    print(f"ℹ️ No reports found for day {i+1}. Skipping.")
                    continue
                
                # Check if adding the new reports would exceed the buffer's token limit.
                # This is a simple approximation (1 token ~= 4 characters).
                # A more precise token count can be obtained using the Gemini SDK.
                approx_tokens = len(str(daily_reports)) / 4
                if combined_token_count + approx_tokens > MAX_BUFFER_TOKENS:
                    print(f"⚠️  Maximum buffer size reached. Skipping remaining days.")
                    break

                print(f"\nGenerating summary for reports from day {i+1}...")
                
                # Use get_user_prompt for daily summaries with the initial system instruction
                daily_prompt = get_user_prompt(daily_reports, output_country)
                system_instruction = get_system_instruction(output_country, output_language)
                
                daily_summary_text = await generate_summary(
                    session, gemini_api_key, gemini_model_name, system_instruction, daily_prompt
                )

                # Write to the in-memory buffer
                daily_summaries_buffer.write(f"\n\n## Summary for Day {i+1}\n")
                daily_summaries_buffer.write(daily_summary_text)

                # Update the token count
                combined_token_count += len(daily_summary_text) / 4


            # --- Step 2: Final "Roll-up" Summary and CVE Enrichment ---
            # Get the full combined summary from the buffer
            combined_summaries = daily_summaries_buffer.getvalue()

            final_report_text = combined_summaries  # Default to combined summaries if no enrichment
            
            if args.enrich_cve and combined_summaries:
                print("\nCombining and generating final roll-up summary and enriching CVEs...")
                
                # Extract all unique CVEs from the combined daily summaries
                cve_pattern = re.compile(r'CVE-\d{4}-\d{4,7}', re.IGNORECASE)
                all_cves = set(cve_pattern.findall(combined_summaries))
                
                cve_details = None
                if all_cves:
                    cve_details = await fetch_vulnerability_details(session, gti_api_key, all_cves)
                
                # A single final Gemini call to create the executive summary and append the table
                final_system_instruction = get_system_instruction(output_country, output_language)
                final_user_prompt = get_final_rollup_prompt(combined_summaries, output_country, cve_details)
                
                final_report_text = await generate_summary(
                    session, gemini_api_key, gemini_model_name, final_system_instruction, final_user_prompt
                )
            
            print("\n" + "="*80)
            print(f"Final Threat Intelligence Summary for {output_country}")
            print("="*80 + "\n")
        ...
        if 'daily_summaries_buffer' in locals():
            daily_summaries_buffer.close()