...
uses Google's Gemini AI model to generate a topic-specific threat summary (filtered by country), 
...
Version: 1.0 (Based on country_specific_gti_summary.py v2.3, added topic filtering)

Usage: python3 topic_specific_gti_summary.py

Arguments:
    -c, --country: The country to focus the report on (default: Singapore).
    -t, --topic: The specific topic or keyword to filter by (e.g., "ransomware", "phishing").
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Generate a topic-specific threat intelligence summary using GTI and Gemini."
    )
    parser.add_argument(
        "-c", "--country",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The specific topic or keyword to filter by (e.g., 'ransomware'). Searches name and description."
    )
    parser.add_argument(
        "-l", "--language",
        type=str,
    ...
    parser.add_argument(
        ...
        help="The Gemini model to use for the summary. Default: gemini-3.5-flash."
    )
...
async def fetch_reports(session, gti_api_key, country, source, topic=None, start_date='4d', end_date='0d', limit=1000):

    print(f"Fetching reports from {start_date} ago to {end_date} ago for {country} (Topic: {topic}, Source: {source})...")
    ...
    # Debug print to verify filter construction
    print(f"DEBUG: Using filter: {full_filter}")
...
def get_system_instruction(output_country, output_language, topic=None):
    ...
    <PROMPT>
        ...
        <INPUT_FORMAT>
            ...
            3.  **`TOPIC` (Optional):** A specific topic to focus on (e.g., "Ransomware").
            4.  **`REPORT_OBJECTS`:** A list of intelligence report objects, primarily with `Origin` of 'Crowdsourced' or 'Partner', but may also include 'News Analysis' reports for perspective. Each object may contain fields such as:
        ...
        3.  **Group and Structure the Output:**
            ...
            * {f"Ensure the structure reflects the focus on {topic}." if topic else ""}
...
def get_user_prompt(collections, output_country, topic=None, cve_details=None):
    """
    Creates the user-facing prompt that includes the fetched report data and optional CVE data.
    """
    today_str = datetime.date.today().strftime("%A, %B %d, %Y")
    
    collections_subset = collections[:350] # Limit to the first 350 reports to avoid exceeding token limits.
    ...
    topic_instruction = f" and related to the topic '{topic}'" if topic else ""

    prompt = f"""
    Create a concise and engaging newsletter for cyber threat intelligence professionals protecting organizations and interests based in {output_country}{topic_instruction}.
...
async def main():
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            collections = await fetch_reports(
                session, 
                gti_api_key, 
                output_country, 
                source_option, 
            ...
            system_instruction = get_system_instruction(output_country, output_language, topic=output_topic)
            user_prompt = get_user_prompt(collections, output_country, topic=output_topic, cve_details=None)
            ...
            ### Final Output ###
            print("\n" + "="*80)
            print(f"Threat Intelligence Summary for {output_country} ({output_topic if output_topic else 'General'})")
            print("="*80 + "\n")
            print(final_report)
